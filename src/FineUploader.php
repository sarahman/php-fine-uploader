...
namespace Sarahman\Utilities;

/**
 * Class FineUploader
 *
 * @package Sarahman\Utilities
...
class FineUploader
{
    ...
    protected function getUniqueTargetPath($uploadDirectory, $filename)
    {
        ...
        if (function_exists('sem_acquire') && isset($lock)) {
            sem_release($lock);
        }
    ...
    {
        $val = trim($str);
        $byteIndex = array('k' => 1, 'm' => 2, 'g' => 3);
        $last = strtolower($str[strlen($str) - 1]);
        $multiplier = pow(1024, isset($byteIndex[$last]) ? $byteIndex[$last] : 0);
        ($multiplier == 1) || $val = substr($val, 0, -1) * $multiplier;
        return $val;
    }
